...
class Graph:
    def __init__(self):
        self.Nodes = read(os.path.join("./config/data.json"))
        ...
        self.Edges = []
        with open(os.path.join("./config/edge.txt"), "r") as f:
            for line in f:
        ...
        self.f = []
        with open(os.path.join("./config/START.json"), "r") as f:
            self.f = json.load(f)
...
if __name__ == '__main__':
    ...
    print(z.id)
    res = Visulize(NodesList, os.path.join("./saveImg/scene1410.jpg"))
    imshow(res)
